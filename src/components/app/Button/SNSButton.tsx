...
function NaverLogo(): React.JSX.Element {
  return (
    <svg width='20' height='20' viewBox='0 0 20 20' fill='none' xmlns='http://www.w3.org/2000/svg'>
      <path d='M12.7143 10.54L7.08571 3H3V17H7.28571V9.46L12.9143 17H17V3H12.7143V10.54Z' fill='#FFFFFF' />
    </svg>
  );
}
...
export function SNSButton({ provider, shape = 'circle', label, onClick, className, style }: SNSButtonProps): React.JSX.Element {
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        'flex items-center justify-center gap-2 w-full h-12 rounded px-4 cursor-pointer text-sm font-medium leading-[22px] tracking-[-0.02em]',
        className
      )}
      style={{
        background: providerStyle.background,
